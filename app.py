...
def load_data():
    try:
        # Load the new 100-employee database
        df = pd.read_csv('wearable_data_100_employees.csv')
        df.columns = df.columns.str.strip()
        
        # Safely convert time to numeric hours for graphing
...
# Extract only the active work shift (where they are actively producing)
...
# Dropdown to select an employee (Now with 100 options!)
...
with colC:
    ...
    # Take only the Top 20 worst cases so the graph is readable
    top_profiles = profiles.head(20).copy()

    # Assign traffic light colors based on their score
    bar_colors = []
...
with colD:
    ...
    # We use the full DF to show the trend across all 100 employees
